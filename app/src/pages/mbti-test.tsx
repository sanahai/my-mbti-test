...
import { mbtiQuestions } from "@/data/test-questions";
import { calculateMBTIResult } from "@/lib/test-scoring";
...
export default function MBTITest() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      toast({
        title: "회원 전용",
        description: "MBTI 테스트는 회원만 이용할 수 있습니다. 로그인 후 이용해주세요.",
        variant: "destructive",
      });
      setLocation(`/login?redirect=${encodeURIComponent('/test/mbti')}`);
    }
  ...
  if (isLoading || !isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-950/20 dark:to-blue-950/20 flex items-center justify-center">
  ...
  return (
    <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4">
      <div className="text-center mb-4">
        <h1 className="font-black text-gray-900 mb-1 text-center leading-tight" style={{fontFamily: 'var(--font-malgun)', fontSize: 'clamp(1.35rem, 4.5vw, 1.875rem)', maxWidth: '100%', overflowWrap: 'break-word', fontWeight: 'bolder', textShadow: '0.5px 0.5px 0px currentColor'}}>MBTI 성격 테스트</h1>
      ...
      <Card className="max-w-4xl mx-auto">
        <CardContent className="p-4">
          <ProgressBar 
            key={`mbti-progress-${currentQuestion}`}
